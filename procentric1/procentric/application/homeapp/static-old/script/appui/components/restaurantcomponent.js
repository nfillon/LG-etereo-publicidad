...
//                                        WG._onSelectHotelInfo(self, e);
            ...
            _createTitleLabel: function() {
                ...
                this._titleLabel.appendChildWidget(new Label("RESERVAS"));
            },
            ...
            _getSuccessModalConfig: function (parent, e) {
                ...
                function _onselectSchedule() {
                    ...
                    restaurant.orderCompleted = true;
                    self.parentWidget.back();
                };

                var datasource = [{'id': '1', 'description': 'OK'}];
                var description = 'Su reserva a ' + restaurant.restaurant + " de una mesa para " + restaurant.tableSeat + " para las " + restaurant.schedule + " ya ha sido enviada.";
                return {
            ...
            _getScheduleConfig: function (parent) {
                var self = parent;
                ...
                function _onselectSchedule(schedule) {
                    try {
                        ...
                        self.getCurrentApplication().pushComponent(
                            ...
                            self._getSuccessModalConfig(self)
                        );
                    } catch (e) {
                        WG.log(e);
                    }
            ...
            _getTableSeatsConfig: function (parent) {
                var self = parent;

                function _onselectSchedule(tableSeats) {
                    try {
                        restaurant.tableSeat = tableSeats;
                        self.getCurrentApplication().pushComponent(
                            "modalcontainer",
                            "homeapp/appui/components/schedulercomponent",
                            self._getScheduleConfig(self)
                        );
                    } catch (e) {
                        WG.log(e);
                    }
                }
                var datasource = [{'id': '1', 'description': '1',},{'id': '2','description': '2',}, {'id': '3','description': '3',
                },{'id': '4','description': '4',}, {'id': '5','description': '5',},{'id': '6','description': '6',}];
                return {
                    home: self, //parametro para poder volver
                    description: "Seleccione cantidad de personas para su reserva",
                    dataSource: datasource,
                    formatter: new SchedulerFormatter({ onSelect: _onselectSchedule}),
                    orientation: Carousel.orientations.VERTICAL,
                    carouselId: 'languagecontainer',
            ...
            _createCarousel: function (args) {
                ...
                function onSelect(appId) {
                    restaurant.restaurant = appId;
                    try {
                        self.getCurrentApplication().pushComponent(
                            "modalcontainer",
                            "homeapp/appui/components/schedulercomponent",
                            self._getTableSeatsConfig(self)
                        );
                    } catch (e) {
                        WG.log(e.stack);
                    }
